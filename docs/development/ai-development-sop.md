...
```text
1. git status：我在哪个分支，有没有用户未提交的改动？
2. Roadmap + Spec：当前 Feature 是什么，状态是否允许实现？
3. ADR + active Plan：哪些决定不能擅自改变，下一步是哪一片？
4. 代码 + 测试：当前行为到底是什么，而不是聊天里曾经说过什么？
5. 变更级别：这是 S0、S1 还是 S2？
```

如果第 2 步找不到已接受的 S1/S2 Spec，先写清范围并等待接受；如果 Git 状态与预期不一致，先保护
现有改动。后面的完整流程是在这五项确认之后展开。
...
flowchart TB
    A["调查：读仓库事实"] --> B{"变更风险多大？"}
    B -->|"S0 小修"| C["修复 + 回归测试"]
    B -->|"S1 行为变化"| D["接受精简 Feature Spec"]
    B -->|"S2 架构/安全/数据"| E["接受完整 Spec + ADR + Plan"]
    D --> F{"需要多片实施吗？"}
    F -->|"是"| P["Plan：拆成可单独验证的步骤"]
    F -->|"否"| G["实现并验证"]
    P --> G
    E --> G
    C --> G
...
| 级别 | 典型改动 | 必需结果 |
|---|---|---|
| S0 | typo、格式、明确 bug、小型内部重构 | 修复和针对性验证；可执行缺陷补回归测试，纯文字/格式不造测试 |
| S1 | 新 CLI、Tool、状态或用户可观察行为 | 精简 Feature Spec、测试；只有多片实施时才写 Plan |
| S2 | 持久 schema、安全边界、跨模块接口、新生产依赖 | 完整 Spec、ADR、active Plan、失败/恢复/安全证据、迁移和回退 |

BearAgent 整体使用 Complex SDD，不表示每次修改都是 S2。ADR 只记录影响多个模块、以后难以反转的
决定，不记录每日实现细节。S0 不补仪式性文档，S1 也不因为模板存在而扩写无关章节。

仓库初始化以后，项目所有者可以继续直接用自然语言提出目标。AI 根据当前 Git、风险和影响选择
S0/S1/S2，并遵守仓库中的规则；项目所有者不需要记忆一组启动命令或重复调用初始化流程。
...
| 当前实施顺序 | 当前 Feature 的 `active` Plan；没有 Plan 的 S1 直接以 Spec 验收条件推进 |
...
### 4.3 第三步：接受大小合适的 Spec

S1/S2 先写 Feature Spec。S1 只需要问题、目标/非目标、可观察行为、必要的失败边界、验收标准和
证据映射。S2 再展开状态、持久化、恢复、安全、迁移和回退。

验收条件不能使用“更智能”“更稳定”“体验良好”等无法判断的词。开放问题不由实现代码暗中决定。
Spec 不维护完整函数签名、文件树或类清单，也不生成一份平行架构百科；这些事实由代码、schema、
CLI help 和测试提供。
...
### 4.5 第五步：需要时用 Plan 拆成可验证步骤

S2 必须有 Plan。S1 只有在需要多个独立验证切片、跨多个提交/PR，或一次完成会让评审失去边界时才
创建 Plan。能在一个连贯变更中实现和验证的 S1，直接按 Spec 推进。

Plan 的每一步都写：交付结果、代码落点、谁调用谁、重点测试、验证命令和回退方式。例如：
...
- 填入不可变的实际 PR/commit，不把临时分支名当作完成证据；
- 更新发生变化的架构事实；
- 对工程文档、学习页、开发者页和状态页逐项写入“更新路径”或“`N/A` + 原因”；
- 学习页只在概念或用户路径变化时更新，开发者页只在代码阅读入口或边界变化时更新；
- 状态页只在可用能力、限制或阶段状态变化时更新；
- 迁移、回退和已知限制写清；
- 完整验证和治理检查通过后提交。
...
长期 Architecture、Roadmap、定位和 SOP 不手写 `version`、`last_verified` 这类无法自动证明的新鲜度
字段；版本和变更时间由 Git 历史回答。Feature/Plan 的 `created`、`last_updated` 和 `implemented_in`
继续记录生命周期，其中完成证据使用不可变 PR/commit。

自动生成只用于可以从仓库确定得到的事实，例如 schema、依赖图、Feature 状态索引和验收证据矩阵。
模块职责、信任边界、失败语义和设计取舍仍需人工审查，不从代码注释批量生成长篇架构正文。
...
- Markdown 内部链接；
- Spec、Plan、ADR、索引和内部 `sourceRefs` 的治理一致性；
- Starlight 生产构建和本地搜索；
- Ruff、Pyright、pytest；
- import boundary；
- schema snapshot；
...
`scripts/check_governance.py` 以 Spec Front Matter 为状态源，检查稳定 ID、状态关系、完成清单、索引和
内部引用。它不会创建第二份 Feature registry。CI 能阻止格式、链接、状态、生成物和契约的确定性
漂移，不能判断一段解释是否自然，因此仍需人工连续阅读。
...
- 合并前从干净环境运行完整验证和 `uv run python scripts/check_governance.py`。
...
```text
开始：读 Roadmap 和当前 Spec；有 active Plan 时选择下一步，否则按验收条件推进
开发：测试 -> 最小实现 -> 范围检查
结束：记录剩余问题，不重写整份架构
Feature：反向审查 -> 文档影响判断 -> 治理检查 -> 完整验证
