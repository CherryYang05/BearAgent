...
BearAgent 的原则是：结果能确认才继续；无法确认就停下并说明情况，不把猜测写成成功。P1、P2、
P3 分别把过程可查、失败恢复、权限与隔离做成可以复现的行为。
...
| P2 失败恢复 | 进程中断后只从能确认的位置继续 | kill point、状态重建、不重复写入、`UNKNOWN` |
| P3 权限与隔离 | 危险动作必须获准，代码与宿主隔离 | 批准参数篡改测试、密钥隔离、备份恢复 |
| P4 日常使用 | Skill、MCP、Web、Memory 依次接入 | 新入口仍经过原有记录和权限路径 |
...
P1 至 P3 固定为单用户、单 Agent、单进程、SQLite 和 CLI 优先。只接一个真实模型和有限文件工具，
所有外部动作都经过统一执行入口，主 Runtime 进程不执行模型生成的 shell。

项目不承诺任意外部写入 exactly-once，也不使用“绝对安全”“永不丢任务”等表达。Web UI、Memory、
任意联网、浏览器控制、多个 Agent、多用户和分布式 worker 都在核心闭环以后。
