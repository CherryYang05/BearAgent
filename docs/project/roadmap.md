...
- 每个 Feature 完成时，必须同步工程 `docs/`、站点初学者学习路径、站点开发者文档和公开当前状态。
- 每个 P 阶段完成时，必须额外同步 Roadmap、学习地图、开发者架构总结和阶段结果，再开始下一阶段。
- 外部 Agent 资料用于教学解释和设计对照；BearAgent 当前能力只由本仓库的 Spec、代码、测试和验收证据确认。
