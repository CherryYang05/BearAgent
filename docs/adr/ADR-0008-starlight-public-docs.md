---
title: "ADR-0008: Use Starlight for the documentation site"
status: accepted
date: 2026-08-10
...
# ADR-0008：文档展示站使用 Starlight

## 要解决的问题

工程 `docs/` 适合记录精确需求和决定，但不适合直接充当初学者教程。文档站需要中文导航、本地
...
F-0015 只交付文档内容、本地预览、静态构建和 CI。站点使用根路径与 `/zh-cn/` 中文路由，不绑定
GitHub Pages、仓库子路径、域名或其他托管平台。普通 PR 和 `main` 都只验证构建，不自动部署。
如果以后需要在线托管，应通过新的范围决定部署目标、权限、域名和上线验收。

## 带来的影响

仓库同时维护 Python/uv 与 Node/npm 两套工具链，CI 需要分别验证。Mermaid 是额外集成，需要关注
版本。得到的好处是本地搜索、中文导航和教学页面可以独立构建、预览，不增加 Runtime 服务，也不
让文档内容依赖某个部署平台。

## 怎样验证

从 lockfile 安装后，生产构建必须生成中文页面、Pagefind 索引、Mermaid 图和中文 404；根页跳转到
`/zh-cn/`，资源从站点根路径加载。构建过程不需要密钥、数据库、用户数据或部署凭据。需要选择
托管平台或维护多个已发布版本时，再重新评估部署方案和框架配置。
