---
title: "ADR-0008: Use Starlight for the documentation site"
status: accepted
date: 2026-08-10
decision_owners: [CherryYang05]
supersedes: null
superseded_by: null
---

# ADR-0008：文档展示站使用 Starlight

## 要解决的问题

工程 `docs/` 适合记录精确需求和决定，但不适合直接充当初学者教程。文档站需要中文导航、本地
搜索、状态提示、Mermaid 和可定制首页，同时不能读取 Runtime 数据或依赖外部搜索服务。

## 选择时最看重什么

- 内容作者主要写 Markdown/MDX，前端代码尽量少；
- 搜索和预览可以完全在本地工作；
- 中文 URL 从第一天稳定在 `/zh-cn/`；
- 产物是静态文件，不需要动态后端；
- 框架仍在积极维护。

## 比较过的方案

- Starlight 提供文档导航、Pagefind 搜索和多语言路由，Astro 允许小范围定制；代价是新增 Node/npm；
- Docusaurus 功能成熟，但 React 工程和外部搜索路径对当前需求偏重；
- VitePress 简洁，但 Vue 定制和版本能力没有带来明显优势；
- Material for MkDocs 已进入维护模式，Zensical 当时仍处于 alpha。

## 决定

站点位于 `site/`，使用 Starlight、npm 和提交的 `package-lock.json`。中文内容放在 `/zh-cn/`，使用
Pagefind 和锁定版本的 Mermaid 集成，不接远程分析或搜索。

工程事实仍由 `docs/`、代码和测试确认。站点按读者任务重写这些事实，不复制 Spec。每个 Feature
完成时更新相关学习页、开发者页和当前状态；每个阶段完成时再更新学习地图和阶段结果。

F-0015 只交付文档内容、本地预览、静态构建和 CI。站点使用根路径与 `/zh-cn/` 中文路由，普通 PR
和 `main` 都只验证构建，不自动部署。线上版本的目标是项目自己的服务器；发布目录、权限、域名、
反向代理、TLS 和上线验收由后续独立范围决定。

## 带来的影响

仓库同时维护 Python/uv 与 Node/npm 两套工具链，CI 需要分别验证。Mermaid 是额外集成，需要关注
版本。得到的好处是本地搜索、中文导航和教学页面可以独立构建、预览，不增加 Runtime 服务，也不
让文档内容依赖托管平台或服务器实现细节。

## 怎样验证

从 lockfile 安装后，生产构建必须生成中文页面、Pagefind 索引、Mermaid 图和中文 404；根页跳转到
`/zh-cn/`，资源从站点根路径加载。构建过程不需要密钥、数据库、用户数据或部署凭据。需要选择
自有服务器或维护多个已发布版本时，再重新评估部署方案和框架配置。
