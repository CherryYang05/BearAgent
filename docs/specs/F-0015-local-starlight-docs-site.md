...
- G-6：每个 Feature 判断工程文档、学习页、开发者页和当前状态的影响；有变化才更新，无变化记录
  `N/A` 和原因；
- G-7：独立 CLI 手册，不让读者从多个 Spec 中拼出使用方法；
- G-8：序章、六部分和附录组成的书籍结构，并用少量 4K 插画建立章节印象；
- G-9：用仓库内治理检查约束 Spec、Plan、ADR、索引、完成清单和站点内部引用，不引入第二份
  Feature registry。

## 3. 本次不做

本次不实现自有服务器部署、域名、反向代理、TLS 或自动发布 workflow；这些由后续部署范围负责。
也不完成英文翻译、历史版本、问答 Agent、评论、登录和分析后台，不改变任何 Runtime 行为。
...
一个 Feature 准备关闭时，必须分别判断工程事实、学习说明、代码导读和当前状态是否受到影响。受到
影响的页面同步更新；没有影响的表面记录 `N/A` 和原因。整个阶段关闭时，再更新学习地图和阶段结果。
...
- FR-10：每个 Feature 对工程 `docs/`、学习页、开发者页和状态页逐项记录更新路径或 `N/A` 原因；
...
- FR-18：生成式插画不承载精确标签、代码或安全规则；入库资源达到 3840×2160，有替代文本，并在
  桌面和手机宽度下不遮挡正文。
- FR-19：治理检查以 Spec Front Matter 为 Feature 状态源，校验稳定 ID、Spec/Plan/ADR 关系、索引、
  完成清单和站点中 ID 形式的 `sourceRefs`；不得创建平行状态 registry。
- FR-20：S1 使用精简 Spec，只有多片实施时才创建 Plan；S2 使用完整 Spec、ADR 和 active Plan。
...
- AC-16：`scripts/check_governance.py` 能拒绝状态不一致、完成文档中的未勾选项、失效内部引用和
  错误索引，并在当前仓库通过；
- AC-17：SOP、Feature/Plan 模板、PR 模板和 `AGENTS.md` 都允许无影响表面使用 `N/A`，不再强制每个
  Feature 修改全部站点页面。
