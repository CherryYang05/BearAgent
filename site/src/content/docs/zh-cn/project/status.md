...
F-0016 和 F-0005 在 Roadmap 中有稳定 Feature ID，但没有 Spec 的条目仍只是待办。仓库当前没有
active Plan；下一个 P1 Feature 需要由项目所有者确认后开始。
...
- 有统一 Tool 执行入口和实际 workspace 读写 Tool，但还没有 Agent Loop、用户 Approval、sandbox 或
  服务器 API；Artifact 元数据也没有写入 Event 或 SQLite；
