...
| P1 可检查执行 | 已完成，2026-09-08 收口 | 文件任务可以完成，模型和工具的过程、预算与失败可以查看 |
| P2 可恢复执行语义 | F-0021 本地实现，待发布验证 | 根据证据恢复或停在 UNKNOWN；用相同故障集比较不同策略 |
...
F-0020 补上配置访问保护、无覆盖初始化和离线检查，已随 PR #24 合入 main。2026-09-08 核对
507 个离线测试、Windows/Linux CI 和 main push 文档部署后，P1 全部 Feature 完成收口。
历史真实 gate 与这次离线回归分别记录，证据入口见[当前状态](/zh-cn/project/status/)。
...
2026-09-08 从 P1 的 `v0.1.0` 基线实现 F-0021：工作分支增加只读 replay/check，从 Event 重建状态
并检查未结束或异常的 Run；实现提交与远程 CI 仍待补齐。它们不执行恢复，以下仍是 P2 阶段目标。
