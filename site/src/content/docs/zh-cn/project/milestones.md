...
| P2 可恢复执行语义 | 设计启动，F-0021 草案 | 根据证据恢复或停在 UNKNOWN；用相同故障集比较不同策略 |
...
2026-09-08 从 P1 的 `v0.1.0` 基线启动 F-0021 设计。第一步只拟议从 Event 重建状态并检查未结束
的 Run；Spec、ADR 与计划接受后才开始实现，以下仍是阶段目标。
