...
如果想先看行业全景，回到[Agent 现在发展到哪一步](agents-today.md)和
[Agent 仍然难在哪里](open-problems.md)。如果要进入 BearAgent 执行细节，继续读
[状态和预算怎样计算](runtime-state-and-budgets.md)，或直接看
[Tool 请求的四道检查](tool-execution-boundary.md)。
