...
| 步骤 | 读者问题 | 读完能做到 |
|---|---|---|
| 第一步：先亲手跑一次 | “这个项目到底怎么用？” | 配置模型，启动 Run，查看 Artifact、状态和 Event |
| 第二步：看懂 Agent 运行 | “Model、Tool、Runtime 各干什么？” | 画出一次 Run 的循环，解释状态和预算来源 |
| 第三步：理解安全边界 | “模型为什么不能直接读写文件？” | 解释 adapter、Policy、路径边界和原子提交 |
| 第四步：拼出系统架构 | “这些模块为什么这样分？” | 看懂 interface、application、runtime、port 和 adapter |
| 第五步：沿代码继续学习 | “从哪个文件开始读，怎么验证？” | 顺着调用链找到实现、契约和失败测试 |
| 第六步：走向研究与扩展 | “今天还有哪些问题没有解决？” | 把恢复、隔离、评测和长任务变成研究问题 |
...
这条学习路线对应 2026-09-08 已收口的 P1：你可以初始化配置、完成有限文件任务，并核对 Event 与
Artifact。中断后自动继续仍属于 P2，当前边界与验收记录见[现在实现到了哪里](/zh-cn/project/status/)。

从[第一次运行：读一份文档](/zh-cn/learn/first-run/)开始，先看到输入、输出文件与执行记录。需要查参数、
退出码和路径时，再打开[完整命令行手册](/zh-cn/guides/cli/)，不用先通读全部参考内容。
...
先读[从一次失败，到一个可以检验的研究问题](/zh-cn/learn/research-experiments/)。它沿同一个文件任务
区分结果核对、原因验证与执行授权，并说明怎样比较不同恢复策略。这些扩展属于 P2/P3 规划。
