...
2. 阅读[BearAgent 架构概览](../architecture/)理解 Core（核心）、Port（内部接口）和 Adapter（适配器）的依赖方向。
...
- [F-0001：内部数据格式实现](domain-contracts.md)：ID、Message、Error、Event 通用外壳和 JSON Schema 快照。
...
- [F-0004：ModelProvider](model-provider.md)：统一内部数据格式、Responses 流式响应翻译与安全失败。
...
| 为什么要做 | Feature Spec |
| 为什么选择这种设计 | ADR |
| 当前做到哪个可单独验证的实现步骤 | Implementation Plan |
