...
5. 按下面的实现导读进入代码和测试；
6. 修改后运行完整验证和治理检查，并为每个文档表面记录更新路径或 `N/A` 原因。
...
- [F-0003：SQLite EventStore](/zh-cn/development/sqlite-event-store/)——transaction、migration、projection 与故障测试；
- [F-0021：只读重建与检查](/zh-cn/development/event-replay/)——EventReplaySource、Reducer、状态 hash、分页、期限与取消；
...
授权/隔离没有实现。F-0021 的 replay/check 已完成收口并通过跨平台 CI，2026-09-13 已合入 main，保持只读；
